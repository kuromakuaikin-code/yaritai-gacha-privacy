...
struct SettingsView: View {
        ...
        NavigationStack {
            List {
                ...
                Section("バックアップ") {
                    ...
                    Text("お守り・御札の登録をJSONファイルとして書き出し・読み込みできます。機種変更時のデータ移行にご利用ください。")
                }
            ...
            .fileExporter(
                ...
                contentType: .json,
                defaultFilename: "omamorireminder-backup"
            ) { _ in }
            .fileImporter(
    ...
    private func handleImport(_ result: Result<URL, Error>) {
        ...
        case .success(let url):
            ...
            importMessage = "\(count)件の登録を読み込みました。"
            showImportResult = true
        case .failure:
            ...
            VStack(spacing: 24) {
                ...
                HStack(spacing: 16) {
                    ForEach(0..<4, id: \.self) { i in
                        Circle()
                            .strokeBorder(Color.omamoriGold, lineWidth: 2)
                            .background(Circle().fill(i < currentInput.count ? Color.omamoriGold : Color.clear))
