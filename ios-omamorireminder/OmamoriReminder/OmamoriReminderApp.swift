...
struct OmamoriReminderApp: App {
    ...
    var body: some Scene {
        ...
        .modelContainer(for: [OmamoriItem.self])
    }
}
...
struct RootTabView: View {
    ...
    var body: some View {
        TabView {
            OmamoriListView()
                .tabItem { Label("一覧", systemImage: "list.bullet.rectangle.fill") }
            GuideView()
                .tabItem { Label("お守りの作法ガイド", systemImage: "book.fill") }
            SettingsView()
                .tabItem { Label("設定", systemImage: "gearshape.fill") }
        }
        .tint(Color.omamoriGold)
                ...
                HStack(spacing: 16) {
                    ForEach(0..<4, id: \.self) { i in
                        Circle()
                            ...
                            .background(Circle().fill(i < input.count ? Color.omamoriGold : Color.clear))
