...
enum AppConfig {
    ...
    /// 無料版で各モジュールごとに登録できる件数の上限（4モジュール共通・独立にカウント）
    ...
    static let premiumProductID = "com.kuromakuaikin.kurashinumbers.premium"
    static let adFreeProductID = "com.kuromakuaikin.kurashinumbers.adfree"

    static let privacyURL = URL(string: "https://kuromakuaikin-code.github.io/yaritai-gacha-privacy/kurashi-number-book/privacy.html")!
    static let termsURL = URL(string: "https://kuromakuaikin-code.github.io/yaritai-gacha-privacy/kurashi-number-book/terms.html")!
}
...
final class PurchaseStore: ObservableObject {
    ...
    /// 広告非表示（広告なし購入 or プレミアム購入のどちらでも true）
    var isAdFree: Bool { adFree || premium }
    /// 4モジュールすべてで件数無制限（プレミアム購入のみ）
...
enum Passcode {
    static func hash(_ code: String) -> String {
        let digest = SHA256.hash(data: Data(("kurashinumbers:" + code).utf8))
