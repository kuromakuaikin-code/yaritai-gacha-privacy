...
struct SettingsView: View {
    ...
    @Query private var utilityRecords: [UtilityRecord]
    @Query private var pointAccounts: [PointAccount]
    @Query private var giftMemories: [GiftMemory]
    @Query private var renewalItems: [RenewalItem]
    ...
    var body: some View {
        NavigationStack {
            List {
                Section("プレミアム") {
                    if store.premium {
                        Label("プレミアム利用中（広告なし・4モジュールすべて件数無制限）", systemImage: "checkmark.seal.fill")
                    ...
                    Text("プレミアム：広告非表示＋光熱費記録・ポイ活残高・記念日・更新期限の4モジュールすべてで件数無制限（無料版はモジュールごとに\(AppConfig.freeItemLimitPerModule)件まで）。広告非表示のみの購入も可能です。")
                }
                ...
                Section("バックアップ") {
                    Button {
                        exportDocument = BackupJSONDocument(
                            data: BackupService.export(
                    ...
                    Text("光熱費・ポイ活残高・記念日・更新期限の4モジュールすべてのデータを、1つのJSONファイルとして書き出し・読み込みできます。機種変更時のデータ移行にご利用ください。")
                }
            ...
            .fileExporter(
                ...
                contentType: .json,
                defaultFilename: "kurashinumbers-backup"
            ) { _ in }
            .fileImporter(
    ...
    private func handleImport(_ result: Result<URL, Error>) {
        ...
        case .success(let url):
            ...
            importMessage = "\(count)件のデータを読み込みました。"
            showImportResult = true
        case .failure:
                ...
                HStack(spacing: 16) {
                    ForEach(0..<4, id: \.self) { i in
                        Circle()
                            ...
                            .background(Circle().fill(i < currentInput.count ? Color.kurashiGold : Color.clear))
