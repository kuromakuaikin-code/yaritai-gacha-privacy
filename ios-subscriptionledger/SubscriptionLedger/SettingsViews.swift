...
struct SettingsView: View {
        ...
        NavigationStack {
            List {
                Section("プレミアム") {
                    ...
                    Text("プレミアム：広告非表示＋登録数無制限（無料版は\(AppConfig.freeSubscriptionLimit)件まで）。広告非表示のみの購入も可能です。")
                }
                ...
                Section("バックアップ") {
                    Button {
                        exportDocument = BackupJSONDocument(data: BackupService.export(subscriptions: subscriptions) ?? Data())
                        showExporter = true
                    } label: {
                    ...
                    Text("登録内容をJSONファイルとして書き出し・読み込みできます。機種変更時のデータ移行にご利用ください。")
                }
            ...
            .fileExporter(
                ...
                contentType: .json,
                defaultFilename: "subscriptionledger-backup"
            ) { _ in }
            .fileImporter(
    ...
    private func handleImport(_ result: Result<URL, Error>) {
        ...
        case .success(let url):
            ...
            importMessage = "\(count)件のサブスクを読み込みました。"
            showImportResult = true
        case .failure:
                ...
                HStack(spacing: 16) {
                    ForEach(0..<4, id: \.self) { i in
                        Circle()
                            ...
                            .background(Circle().fill(i < currentInput.count ? Color.brand : Color.clear))
