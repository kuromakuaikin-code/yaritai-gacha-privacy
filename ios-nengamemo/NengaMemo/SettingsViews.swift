...
struct SettingsView: View {
    ...
    @Query private var giftLogs: [GiftLog]
    ...
    var body: some View {
        NavigationStack {
            List {
                Section("プレミアム") {
                    if store.premium {
                        Label("プレミアム利用中（広告なし・宛先数無制限）", systemImage: "checkmark.seal.fill")
                    ...
                    Text("プレミアム：広告非表示＋宛先数無制限（無料版は\(AppConfig.freeContactLimit)件まで）。広告非表示のみの購入も可能です。")
                }
                ...
                Section("バックアップ") {
                    Button {
                        exportDocument = BackupJSONDocument(data: BackupService.export(contacts: contacts) ?? Data())
                        showExporter = true
                    } label: {
                    ...
                    Text("宛先・贈答記録をJSONファイルとして書き出し・読み込みできます。機種変更時のデータ移行にご利用ください。第三者の氏名・住所を含むため、ファイルの管理は安全な場所で行ってください。")
                }
            ...
            .fileExporter(
                ...
                contentType: .json,
                defaultFilename: "nengamemo-backup"
            ) { _ in }
            .fileImporter(
    ...
    private func handleImport(_ result: Result<URL, Error>) {
        ...
        case .success(let url):
            ...
            importMessage = "\(count)件の宛先を読み込みました。"
            showImportResult = true
        case .failure:
            ...
            VStack(spacing: 24) {
                ...
                HStack(spacing: 16) {
                    ForEach(0..<4, id: \.self) { i in
                        Circle()
                            .strokeBorder(Color.nengaRed, lineWidth: 2)
                            .background(Circle().fill(i < currentInput.count ? Color.nengaRed : Color.clear))
