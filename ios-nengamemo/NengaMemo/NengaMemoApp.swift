...
struct NengaMemoApp: App {
    ...
    var body: some Scene {
        ...
        .modelContainer(for: [Contact.self, GiftLog.self])
    }
}
...
struct RootTabView: View {
    ...
    var body: some View {
        TabView {
            ContactListView()
                .tabItem { Label("宛先一覧", systemImage: "person.crop.rectangle.stack.fill") }
            ThisYearView()
                .tabItem { Label("今年の記録", systemImage: "checklist") }
            SettingsView()
                ...
                HStack(spacing: 16) {
                    ForEach(0..<4, id: \.self) { i in
                        Circle()
                            ...
                            .background(Circle().fill(i < input.count ? Color.nengaRed : Color.clear))
