...
struct ManabiRecordApp: App {
    ...
    var body: some Scene {
        ...
        .modelContainer(for: [
            LessonCourse.self, LessonSession.self,
            BookRecord.self,
            StudyGoal.self, StudyLog.self,
            SchoolEvent.self, ChecklistEntry.self
        ])
    }
}
...
struct RootTabView: View {
    ...
    var body: some View {
        TabView {
            LessonCourseListView()
                .tabItem { Label("習い事", systemImage: "figure.walk.circle.fill") }
            BookListView()
                .tabItem { Label("読書記録", systemImage: "book.closed.fill") }
            StudyGoalListView()
                .tabItem { Label("勉強進捗", systemImage: "chart.bar.fill") }
            SchoolEventListView()
                .tabItem { Label("行事チェック", systemImage: "checklist") }
            SettingsView()
                ...
                HStack(spacing: 16) {
                    ForEach(0..<4, id: \.self) { i in
                        Circle()
                            ...
                            .background(Circle().fill(i < input.count ? Color.studyBlue : Color.clear))
