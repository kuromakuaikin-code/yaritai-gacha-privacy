...
struct SettingsView: View {
    ...
    @Query private var courses: [LessonCourse]
    @Query private var books: [BookRecord]
    @Query private var goals: [StudyGoal]
    @Query private var events: [SchoolEvent]
    ...
    var body: some View {
        NavigationStack {
            List {
                Section("プレミアム") {
                    if store.premium {
                        Label("プレミアム利用中（広告なし・全モジュール無制限）", systemImage: "checkmark.seal.fill")
                    ...
                    Text("プレミアム：広告非表示＋習い事・読書記録・勉強進捗・行事チェックの全4モジュールが登録数無制限（無料版は各モジュール\(AppConfig.freeItemLimitPerModule)件まで）。広告非表示のみの購入も可能です。")
                }
                ...
                Section("バックアップ") {
                    Button {
                        exportDocument = BackupJSONDocument(
                            data: BackupService.export(courses: courses, books: books, goals: goals, events: events) ?? Data()
                        )
                        showExporter = true
                    } label: {
                    ...
                    Text("習い事・読書記録・勉強進捗・行事チェック、4モジュールすべてのデータを1つのJSONファイルとして書き出し・読み込みできます。機種変更時のデータ移行にご利用ください。お子さまの記録が含まれるため、書き出したファイルの管理は安全な場所で行ってください。")
                }
            ...
            .fileExporter(
                ...
                contentType: .json,
                defaultFilename: "manabirecord-backup"
            ) { _ in }
            .fileImporter(
            ...
            VStack(spacing: 24) {
                ...
                HStack(spacing: 16) {
                    ForEach(0..<4, id: \.self) { i in
                        Circle()
                            .strokeBorder(Color.studyBlue, lineWidth: 2)
                            .background(Circle().fill(i < currentInput.count ? Color.studyBlue : Color.clear))
