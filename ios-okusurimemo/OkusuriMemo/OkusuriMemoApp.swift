...
struct OkusuriMemoApp: App {
    ...
    var body: some Scene {
        ...
        .modelContainer(for: [FamilyMember.self, Medication.self, IntakeLog.self])
    }
}
...
struct RootTabView: View {
    ...
    var body: some View {
        TabView {
            TodayIntakeView()
                .tabItem { Label("今日の服薬", systemImage: "pills.fill") }
            MedicationListView()
                .tabItem { Label("お薬一覧", systemImage: "list.bullet.clipboard.fill") }
            SettingsView()
                ...
                HStack(spacing: 16) {
                    ForEach(0..<4, id: \.self) { i in
                        Circle()
                            ...
                            .background(Circle().fill(i < input.count ? Color.appTint : Color.clear))
