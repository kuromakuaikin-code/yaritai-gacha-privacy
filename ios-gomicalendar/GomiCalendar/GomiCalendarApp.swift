...
struct GomiCalendarApp: App {
    ...
    var body: some Scene {
        ...
        .modelContainer(for: [GarbageRule.self])
    }
}
...
struct RootTabView: View {
    ...
    var body: some View {
        TabView {
            TodayWeekView()
                .tabItem { Label("今日・今週", systemImage: "calendar") }
            RulesListView()
                .tabItem { Label("ルール設定", systemImage: "list.bullet.clipboard.fill") }
            SettingsView()
    ...
    /// アプリがアクティブになるたびに、当日該当するカテゴリでリマインダー内容を作り直す（ベストエフォート）
    private func refreshReminder() {
        guard reminderEnabled, store.isUnlimited else { return }
                ...
                HStack(spacing: 16) {
                    ForEach(0..<4, id: \.self) { i in
                        Circle()
                            ...
                            .background(Circle().fill(i < input.count ? Color.gomiTint : Color.clear))
