...
// MARK: - テーマカラー（鳥居の朱色 #B23B3B 相当）

extension Color {
    static let toriiRed = Color(red: 0.698, green: 0.231, blue: 0.231)
}

@main
struct GoshuinLogApp: App {
    ...
    var body: some Scene {
        ...
        .modelContainer(for: [GoshuinEntry.self])
    }
}
                ...
                HStack(spacing: 16) {
                    ForEach(0..<4, id: \.self) { i in
                        Circle()
                            ...
                            .background(Circle().fill(i < input.count ? Color.toriiRed : Color.clear))
