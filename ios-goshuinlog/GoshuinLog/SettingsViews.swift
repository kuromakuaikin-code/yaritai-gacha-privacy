...
struct SettingsView: View {
    ...
    @Query private var entries: [GoshuinEntry]
        ...
        NavigationStack {
            List {
                Section("プレミアム") {
                    ...
                    Text("プレミアム：広告非表示＋記録数無制限（無料版は\(AppConfig.freeRecordLimit)件まで）。広告非表示のみの購入も可能です。")
                }
                ...
                Section("バックアップ") {
                    Button {
                        exportDocument = BackupJSONDocument(data: BackupService.export(entries: entries) ?? Data())
                        showExporter = true
                    } label: {
                    ...
                    Text("御朱印の参拝記録をJSONファイルとして書き出し・読み込みできます。機種変更時のデータ移行にご利用ください。")
                }
            ...
            .fileExporter(
                ...
                contentType: .json,
                defaultFilename: "goshuinlog-backup"
            ) { _ in }
            .fileImporter(
            ...
            VStack(spacing: 24) {
                ...
                HStack(spacing: 16) {
                    ForEach(0..<4, id: \.self) { i in
                        Circle()
                            .strokeBorder(Color.toriiRed, lineWidth: 2)
                            .background(Circle().fill(i < currentInput.count ? Color.toriiRed : Color.clear))
