...
enum AppConfig {
    ...
    /// 無料版で各モジュールに登録できる件数の上限（4モジュールそれぞれ独立してカウント）
    ...
    static let premiumProductID = "com.kuromakuaikin.kurashianshin.premium"
    static let adFreeProductID = "com.kuromakuaikin.kurashianshin.adfree"

    static let privacyURL = URL(string: "https://kuromakuaikin-code.github.io/yaritai-gacha-privacy/kurashi-anshin-book/privacy.html")!
    static let termsURL = URL(string: "https://kuromakuaikin-code.github.io/yaritai-gacha-privacy/kurashi-anshin-book/terms.html")!
}

// MARK: - テーマカラー

extension Color {
    /// 落ち着いたセージグリーン（#5F8F6E）。4モジュール共通のティントカラー
    static let kurashiSage = Color(red: 0x5F / 255.0, green: 0x8F / 255.0, blue: 0x6E / 255.0)
}
...
final class PurchaseStore: ObservableObject {
    ...
    /// モジュール共通の無料枠チェック（テスト期間中・プレミアム購入済みは常に true）
    func canAddMore(currentCount: Int) -> Bool {
        AppConfig.freeTrial || isUnlimited || currentCount < AppConfig.freeItemLimitPerModule
    }
...
enum Passcode {
    static func hash(_ code: String) -> String {
        let digest = SHA256.hash(data: Data(("kurashianshin:" + code).utf8))
