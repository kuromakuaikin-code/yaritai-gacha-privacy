    ...
    var body: some View {
        NavigationStack {
            List {
                Section("プレミアム") {
                    if store.premium {
                        Label("プレミアム利用中（広告なし・全モジュール登録数無制限）", systemImage: "checkmark.seal.fill")
                            .foregroundStyle(.kurashiSage)
                    } else {
                        Button {
                    ...
                    Text("プレミアム：広告非表示＋4つのモジュール（家電保証・メンテナンス・植物・在庫管理）すべてで登録数無制限（無料版は各モジュール\(AppConfig.freeItemLimitPerModule)件まで）。広告非表示のみの購入も可能です。")
                }
                ...
                Section("バックアップ") {
                    Button {
                        exportDocument = BackupJSONDocument(data: BackupService.export(
                            appliances: appliances, tasks: tasks, plants: plants, supplies: supplies
                        ) ?? Data())
                        showExporter = true
                    } label: {
                    ...
                    Text("家電保証・メンテナンス・植物・在庫管理、4モジュールすべてのデータを1つのJSONファイルとして書き出し・読み込みできます。機種変更時のデータ移行にご利用ください。")
                }
            ...
            .fileExporter(
                ...
                contentType: .json,
                defaultFilename: "kurashianshin-backup"
            ) { _ in }
            .fileImporter(
            ...
            VStack(spacing: 24) {
                ...
                HStack(spacing: 16) {
                    ForEach(0..<4, id: \.self) { i in
                        Circle()
                            .strokeBorder(Color.kurashiSage, lineWidth: 2)
                            .background(Circle().fill(i < currentInput.count ? Color.kurashiSage : Color.clear))
