...
@main
struct KurashiAnshinBookApp: App {
    ...
    var body: some Scene {
        ...
        .modelContainer(for: [Appliance.self, MaintenanceTask.self, Plant.self, Supply.self])
    }
}
...
struct RootTabView: View {
    ...
    var body: some View {
        TabView {
            ApplianceListView()
                .tabItem { Label("家電保証", systemImage: "washer.fill") }
            MaintenanceListView()
                .tabItem { Label("メンテナンス", systemImage: "wrench.and.screwdriver.fill") }
            PlantListView()
                .tabItem { Label("植物", systemImage: "leaf.fill") }
            SupplyListView()
                .tabItem { Label("在庫管理", systemImage: "shippingbox.fill") }
            SettingsView()
                .tabItem { Label("設定", systemImage: "gearshape.fill") }
        }
        .tint(Color.kurashiSage)
        ...
        .task {
            PresetSeeder.seedMaintenanceTasksIfNeeded(context: context)
        }
    }
}
                ...
                HStack(spacing: 16) {
                    ForEach(0..<4, id: \.self) { i in
                        Circle()
                            ...
                            .background(Circle().fill(i < input.count ? Color.kurashiSage : Color.clear))
