    ...
    var body: some View {
        NavigationStack {
            List {
                Section("プレミアム") {
                    if store.premium {
                        Label("プレミアム利用中（広告なし・カスタム項目無制限）", systemImage: "checkmark.seal.fill")
                    ...
                    Text("プレミアム：広告非表示＋カスタム項目無制限（無料版はカスタム項目\(AppConfig.freeCustomItemLimit)件まで。標準の防災グッズ項目は無料版でも無制限に使えます）。広告非表示のみの購入も可能です。")
                }
                ...
                Section("バックアップ") {
                    Button {
                        exportDocument = BackupJSONDocument(data: BackupService.export(items: checklistItems, contacts: contacts) ?? Data())
                        showExporter = true
                    } label: {
                    ...
                    Text("チェックリストと連絡先カードをJSONファイルとして書き出し・読み込みできます。機種変更時のデータ移行にご利用ください。")
                }
            ...
            .fileExporter(
                ...
                contentType: .json,
                defaultFilename: "bousaicheck-backup"
            ) { _ in }
            .fileImporter(
                ...
                HStack(spacing: 16) {
                    ForEach(0..<4, id: \.self) { i in
                        Circle()
                            ...
                            .background(Circle().fill(i < currentInput.count ? Color.bousaiAmber : Color.clear))
