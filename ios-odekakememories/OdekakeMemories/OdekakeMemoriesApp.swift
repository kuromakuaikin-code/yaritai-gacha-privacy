...
struct OdekakeMemoriesApp: App {
    ...
    var body: some Scene {
        ...
        .modelContainer(for: [OutingVisit.self])
    }
}
...
struct RootTabView: View {
    ...
    var body: some View {
        TabView {
            ParkView()
                .tabItem { Label(OutingModule.park.tabLabel, systemImage: OutingModule.park.icon) }
            SceneryView()
                .tabItem { Label(OutingModule.scenery.tabLabel, systemImage: OutingModule.scenery.icon) }
            CampView()
                .tabItem { Label(OutingModule.camp.tabLabel, systemImage: OutingModule.camp.icon) }
            SightseeingView()
                .tabItem { Label(OutingModule.sightseeing.tabLabel, systemImage: OutingModule.sightseeing.icon) }
            SettingsView()
                .tabItem { Label("設定", systemImage: "gearshape.fill") }
        }
        .tint(Color.odekakeTint)
            ...
            VStack(spacing: 24) {
                ...
                HStack(spacing: 16) {
                    ForEach(0..<4, id: \.self) { i in
                        Circle()
                            .strokeBorder(Color.odekakeTint, lineWidth: 2)
                            .background(Circle().fill(i < input.count ? Color.odekakeTint : Color.clear))
