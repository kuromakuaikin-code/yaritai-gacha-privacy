...
struct SettingsView: View {
    ...
    @Query private var visits: [OutingVisit]
    ...
    var body: some View {
        NavigationStack {
            List {
                Section("プレミアム") {
                    if store.premium {
                        Label("プレミアム利用中（広告なし・記録数無制限）", systemImage: "checkmark.seal.fill")
                            .foregroundStyle(Color.odekakeTint)
                    } else {
                        Button {
                    ...
                    Text("プレミアム：広告非表示＋記録数無制限（無料版は4カテゴリ合計\(AppConfig.freeTotalLimit)件まで）。広告非表示のみの購入も可能です。")
                }
                ...
                Section("バックアップ") {
                    Button {
                        exportDocument = BackupJSONDocument(data: BackupService.export(visits: visits) ?? Data())
                        showExporter = true
                    } label: {
                    ...
                    Text("公園・花見/紅葉・キャンプ/BBQ・観光の全記録をJSONファイルとして書き出し・読み込みできます。機種変更時のデータ移行にご利用ください。")
                }
            ...
            .fileExporter(
                ...
                contentType: .json,
                defaultFilename: "odekakememories-backup"
            ) { _ in }
            .fileImporter(
                ...
                HStack(spacing: 16) {
                    ForEach(0..<4, id: \.self) { i in
                        Circle()
                            ...
                            .background(Circle().fill(i < currentInput.count ? Color.odekakeTint : Color.clear))
