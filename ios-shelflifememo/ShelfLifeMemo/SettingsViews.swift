...
struct SettingsView: View {
        ...
        NavigationStack {
            List {
                Section("プレミアム") {
                    ...
                    Text("プレミアム：広告非表示＋登録数無制限（無料版は\(AppConfig.freeItemLimit)件まで）。広告非表示のみの購入も可能です。")
                }
                ...
                Section("バックアップ") {
                    Button {
                        exportDocument = BackupJSONDocument(data: BackupService.export(items: items) ?? Data())
                        showExporter = true
                    } label: {
                    ...
                    Text("登録した食品をJSONファイルとして書き出し・読み込みできます。機種変更時のデータ移行にご利用ください。")
                }
            ...
            .fileExporter(
                ...
                contentType: .json,
                defaultFilename: "shelflifememo-backup"
            ) { _ in }
            .fileImporter(
    ...
    private func handleImport(_ result: Result<URL, Error>) {
        ...
        case .success(let url):
            ...
            importMessage = "\(count)件の食品を読み込みました。"
            showImportResult = true
        case .failure:
            ...
            VStack(spacing: 24) {
                ...
                HStack(spacing: 16) {
                    ForEach(0..<4, id: \.self) { i in
                        Circle()
                            .strokeBorder(Color.shelfGreen, lineWidth: 2)
                            .background(Circle().fill(i < currentInput.count ? Color.shelfGreen : Color.clear))
