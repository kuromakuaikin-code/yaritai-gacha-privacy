    ...
    var body: some View {
        NavigationStack {
            List {
                ...
                Section("バックアップ") {
                    Button {
                        exportDocument = BackupJSONDocument(data: BackupService.export(records: records) ?? Data())
                        showExporter = true
                    } label: {
                    ...
                    Text("記録をJSONファイルとして書き出し・読み込みできます。機種変更時のデータ移行にご利用ください。")
                }
            ...
            .fileExporter(
                ...
                contentType: .json,
                defaultFilename: "shugimemo-backup"
            ) { _ in }
            .fileImporter(
                ...
                HStack(spacing: 16) {
                    ForEach(0..<4, id: \.self) { i in
                        Circle()
                            ...
                            .background(Circle().fill(i < currentInput.count ? Color.indigo : Color.clear))
