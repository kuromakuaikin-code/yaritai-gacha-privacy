...
struct RootTabView: View {
    ...
    var body: some View {
        TabView {
            RecordListView()
                .tabItem { Label("記録", systemImage: "list.bullet.rectangle.fill") }
            GuideView()
                .tabItem { Label("相場ガイド", systemImage: "yensign.circle.fill") }
            StatsView()
                ...
                HStack(spacing: 16) {
                    ForEach(0..<4, id: \.self) { i in
                        Circle()
                            ...
                            .background(Circle().fill(i < input.count ? Color.indigo : Color.clear))
