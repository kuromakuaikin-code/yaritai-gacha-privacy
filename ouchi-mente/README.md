...
1. App Store Connect でアプリを作成（Bundle ID: `com.ienoteire.app`）
2. 「App内課金」で非消耗型 `com.ienoteire.app.unlimited`（¥300）を作成
...
開発用機能はリリース前にすべて削除済み。Expo Go用のIAPスタブのみ
環境ガードとして残っている（実機ビルドでは実行されない）。
...
- 無制限版: **¥300 買い切り（非消耗型）で登録上限なし**
  - 商品ID: `com.ienoteire.app.unlimited`
...
- 商品ID: `com.ienoteire.app.unlimited`（iOS / Android とも同じID）
