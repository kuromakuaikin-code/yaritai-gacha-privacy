# おうちメンテ目安メモ（Expo / React Native）

掃除・交換・点検の「そろそろ」を忘れないための記録アプリ。
エアコンフィルター、洗濯槽、火災警報器の電池などの実施日を記録すると、
次回の目安日を自動計算し、ローカル通知でお知らせします。
...
- expo-sqlite（メインデータ。端末内のみ、サーバー・外部API・ログインなし）
- expo-notifications（ローカル通知のみ）
- expo-image-picker + expo-file-system（写真は端末内に保存）
...
src/
  ...
  notifications/            # ローカル通知の登録・再設定・破棄
  media/                    # 写真の端末内保存・削除
...
- 1項目につき通知は最大1件。作成・編集・完了記録・削除のたびに
  古い通知IDをキャンセルして登録し直す
- 通知時刻は「目安日 −（通知タイミング日数）」の朝9時。過去時刻なら登録しない
...
- 無料版: 登録上限 **5件**
- 追加枠: **¥300 買い切り（非消耗型）で +10件 → 合計15件**
  - 商品ID: `com.kuromakuaikin.ouchimente.plus10`
