...
struct KodomoGrowthApp: App {
    ...
    var body: some Scene {
        ...
        .modelContainer(for: [Child.self, GrowthRecord.self])
    }
}
...
extension Color {
    /// アプリのテーマカラー（性別を問わないティール系）
    static let kodomoTeal = Color(red: 0x4F / 255.0, green: 0xA0 / 255.0, blue: 0xB0 / 255.0)
}
...
struct RootTabView: View {
    ...
    var body: some View {
        TabView {
            RecordListView()
                .tabItem { Label("記録一覧", systemImage: "list.bullet.rectangle.fill") }
            GrowthChartView()
                .tabItem { Label("グラフ", systemImage: "chart.xyaxis.line") }
            SettingsView()
                .tabItem { Label("設定", systemImage: "gearshape.fill") }
        }
        .tint(Color.kodomoTeal)
                ...
                HStack(spacing: 16) {
                    ForEach(0..<4, id: \.self) { i in
                        Circle()
                            ...
                            .background(Circle().fill(i < input.count ? Color.kodomoTeal : Color.clear))
