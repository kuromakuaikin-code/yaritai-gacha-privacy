...
SwiftUI + AVFoundation の縦動画（9:16）編集アプリ MVP。シニア・非クリエイター向けの**かんたんモード**（えらぶ→ととのえる→ほぞんの3画面一本道、大ボタン・専門用語なし）が既定で、従来のフル編集は「しっかり編集」として切替可能。

- ソース・セットアップ手順: `shortlab/`（`shortlab/README.md` 参照。XcodeGen 推奨）
- 実装済み: かんたんモード / 取り込み / カット・結合 / トリム / 分割 / 速度変更 / テキスト焼き込み / BGM / 1080p書き出し（透かし対応）/ 写真保存・共有 / undo / プロジェクト自動保存・復元 / 買い切り課金（StoreKit 2・¥600想定・透かし解除+広告非表示）/ AdMob バナー掲載口（保存完了画面のみ）
- 開発ルール・受け入れ条件: `shortlab/CLAUDE.md`
- リリース設定（IAP 製品ID・AdMob ID は `shortlab/Config/Store.local.xcconfig` に記入）: `shortlab/README.md`「リリース設定」参照
- 未実装（残り）: スタンプ（素材待ち）/ BGM同梱素材 / AdMob実配信の実機確認
