...
盆踊りナビ・おでかけナビの2サイトを、Threads の1アカウントに統合して運用する設計。**まだ設計書だけで、実装も運用も始めていない。**

- 設計書: `sns/STRATEGY.md`
- 自動化の範囲: **下書き生成のみ**（投稿は手動。API 連携なし）
- AI の役割: ネタの選定と文面の装飾のみ。日付・会場・中止情報は JSON からテンプレートで機械的に埋め、**AI に事実を書かせない**

### 設計時に分かった問題

投稿ネタを月別に数えると、**1月・2月・5月が0件、9月・10月がほぼ0件**だった。年間在庫221件の7割が7〜8月に集中している。README の年間カレンダーは設計としては正しいが、`spots.json` のイルミネーションが1件・あじさいが2件しか入っておらず、データが未投入の状態。
